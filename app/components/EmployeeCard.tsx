...
const EmployeeCard: React.FC<Props> = ({ user }: Props) => {
  return (
    <article className="bg-white flex gap-2 items-end justify-between border border-slate-200 rounded">
      <div className="space-y-2 p-2">
        ...
        <h4
          className={clsx(" font-semibold text-sm px-1 rounded-md capitalize", {
            "bg-green-100 text-green-500":
              user.todayStatus === "clock-from-office",
            "bg-red-100 text-red-500": user.todayStatus === "absent",
            "bg-amber-100 text-amber-500":
              user.todayStatus === "work-from-home",
        ...
        <Link
          href={`/employee/${user.id}/attendance`}
