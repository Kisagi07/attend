...
const Layout: React.FC<Layout> = ({ children }) => {
  ...
  return (
    <>
      <nav className={`p-2  border-b h-14  border-slate-200 bg-white text-slate-600 shadow flex items-center`}>
        <h1 className="text-xl font-sixtyfour uppercase italic font-bold">Attendance</h1>
        <button onClick={() => setSidebarOpen(!sidebarOpen)} type="button" className="ml-auto hover:bg-slate-100 p-2 lg:hidden" title="open sidebar">
